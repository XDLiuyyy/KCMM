...
def save_results_cf_inference(logits_matrix, targets_list,
                 class_to_idx, args, weight):
    """
    Saves the predicted logits matrix, true labels, sample ids and class
    dictionary for further analysis of results
    """
    mystr = str(round(float(weight), 1))
    mystr = mystr.replace('.', '_')
    print("Saving inference results ...")
    path_to_save = os.path.join(
        args.ckpt, args.logname + '_' + mystr + '_' + "cf_inference_results.pkl")
...
def _normalize(my_tensor):
    my_tensor -= my_tensor.min(1, keepdim=True)[0]
    my_tensor /= my_tensor.max(1, keepdim=True)[0]
    return my_tensor


def _rescale_sum_one(my_tensor):
    my_tensor -= my_tensor.min(1, keepdim=True)[0]
    my_tensor /= my_tensor.sum(1, keepdim=True)[0]
    return my_tensor
...
class PPPrint:
    ...
    def __call__(self, msg):
        ...
        print(frameinfo.filename, frameinfo.lineno,":", msg)
