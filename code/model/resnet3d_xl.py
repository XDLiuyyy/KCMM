...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.maxpool2 = nn.MaxPool3d(kernel_size=(2, 1, 1), stride=(2, 1, 1), padding=(0, 0, 0))  # 原本这里的 maxpool 步长是2,即在时间维度上做一个2倍的降采样，我修改成不降采样。但不确定是否会对预训练的参数造成一定影响
    ...
    def forward_single(self, x):
        x = self.conv1(x)  # RGB:3 -> 64, HW: 224 -> 112

        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool1(x)  # HW: 112 -> 55

        x = self.layer1(x)  # channel: 64 -> 256
        x = self.maxpool2(x)  # frames: 16 -> 8
        x = self.layer2(x)  # HW: 55 -> 28, channel: 256 -> 512
...
def Net(num_classes, extract_features=False, loss_type='softmax',
    ...
    if weights is not None:
        ...
        kinetics_weights = torch.load('/home/bqy/CAction/cdn/code/model/pretrained_weights/kinetics-res50.pth')
        cls_name = 'fc'
        print('\n Restoring Kintetics \n')
    ...
    if freeze_all_but_cls:
        for name, par in net.named_parameters():
            # if not name.startswith('classifier'):  # NOTE: 这里我把分类层也 freeze 了
            par.requires_grad = False
    return net

if __name__ == '__main__':
    i3d = Net(171, extract_features=True, loss_type='softmax', freeze_all_but_cls=True).cuda()
    inp = torch.rand((72, 3, 16, 224, 224)).cuda()
    out = i3d(inp)
    print('out size is: ', out.shape)
